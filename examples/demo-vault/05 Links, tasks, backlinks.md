...
Fetch backlinks to a specific note. The cell below calls `vault.backlinks()` to find all notes that link to `library/Dune.md`. Two notes in the vault link to Dune: `library/Reading list.md` and `library/Notes on sci-fi.md`.

```marimo
backlinks = await vault.backlinks("library/Dune.md")
[bl.path for bl in backlinks]
```

To verify these results match what you see in Obsidian, open the backlinks pane for `library/Dune.md` inside Obsidian and compare it with the list above. Both sources will show the same two notes.
...
Find unresolved links on a note. Unresolved links point at files that do not exist. The cell below fetches the unresolved links on `library/Reading list.md`. The note references `[[Foundation]]`, which is not a file in the vault.
